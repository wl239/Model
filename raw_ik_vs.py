...
result1 = comparison_regression(model_list, file_name, x_reg_train, x_reg_test, y_reg_train, y_reg_test)
result2 = comparison_classification(model_list, file_name, 'buy', x_buy_train, x_buy_test, y_buy_train, y_buy_test)
result3 = comparison_classification(model_list, file_name, 'equity', x_equity_train, x_equity_test, y_equity_train, y_equity_test)

df_result1 = pd.DataFrame(result1, columns=['Method', 'MSE', 'R^2']).set_index(['Method'])
df_result2 = pd.DataFrame(result2, columns=['Method', 'Accuracy 1']).set_index(['Method'])
df_result3 = pd.DataFrame(result3, columns=['Method', 'Accuracy 2']).set_index(['Method'])
df_all_result = pd.concat([df_result1, df_result2, df_result3], axis=1)

# MSE, R square, Accuracy Table
save_dataframe_as_png(df_result1, file_path + '_regression.png')
save_dataframe_as_png(df_result2, file_path + '_buy_sell_class.png')
save_dataframe_as_png(df_result3, file_path + '_E_F_class.png')
